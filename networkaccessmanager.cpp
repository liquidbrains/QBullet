...
NetworkAccessManager::NetworkAccessManager(QObject *parent)
    ...
    requestFinishedSecureCount(0), requestFinishedDownloadBufferCount(0),settings(new QSettings(QCoreApplication::organizationName(),QCoreApplication::applicationName(),this))
{
...
void NetworkAccessManager::loadSettings()
{
    ...
    if (settings->value(QLatin1String("type"), QNetworkProxy::NoProxy).toInt() != QNetworkProxy::NoProxy) {
        qDebug() << "Proxy type: "<< settings->value(QLatin1String("type"), QNetworkProxy::NoProxy).toInt();
        switch(settings->value(QLatin1String("type"), QNetworkProxy::NoProxy).toInt())
        {
        case (int)QNetworkProxy::HttpProxy:
            proxy = QNetworkProxy::HttpProxy;
            break;
        case (int)QNetworkProxy::Socks5Proxy:
...
void NetworkAccessManager::authenticationRequired(QNetworkReply *reply, QAuthenticator *auth)
{
    ...
    passwordDialog.iconLabel->setPixmap(((QWidget*)QNetworkAccessManager::parent())->style()->standardIcon(QStyle::SP_MessageBoxQuestion, 0, ((QWidget*)QNetworkAccessManager::parent())).pixmap(32, 32));
...
void NetworkAccessManager::proxyAuthenticationRequired(const QNetworkProxy &proxy, QAuthenticator *auth)
{
    ...
    proxyDialog.iconLabel->setPixmap(((QWidget*)QNetworkAccessManager::parent())->style()->standardIcon(QStyle::SP_MessageBoxQuestion, 0, ((QWidget*)QNetworkAccessManager::parent())).pixmap(32, 32));
...
void NetworkAccessManager::sslErrors(QNetworkReply *reply, const QList<QSslError> &error)
{
    ...
    if(! sslTrustedHostList.contains(replyHost)) {
        ...
        int ret = QMessageBox::warning(parent->isVisible()?parent:NULL, QCoreApplication::applicationName(),
                tr("SSL Errors:\n\n%1\n\n%2\n\n"
